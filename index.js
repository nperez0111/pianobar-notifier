...
loadJsonFile( 'settings.json' ).then( json => {
    ...
    loadJsonFile( '/Users/nickthesick/.config/pianobar/cur.json' ).then( prefs => {
        ...
        } ).catch( err => { console.err( err ) } ) )( { albumArt: prefs.coverArt, url: prefs.detailUrl, artist: prefs.artist, song: prefs.title, album: prefs.album, radio: prefs.songStationName || prefs.stationName, icon: json.icon } )
    } )
} )
